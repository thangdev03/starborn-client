...
const Subcategories = () => {
    ...
    if (
      data?.find(
        (subcategory) =>
          ...
          subcategory.category_id === newSubcategory.categoryId
      )
    ) {
      return toast.warn("Danh mục sản phẩm này đã tồn tại!");
    } else {
      axios
        .post(serverUrl + "categories/sub", {
  ...
  const handleDeleteSubcategory = (subcategory) => {
    if (
      window.confirm(
        'Bạn có chắc muốn xóa tiểu danh mục sản phẩm "' +
          subcategory.name +
          "-" +
      ...
      axios
        .delete(serverUrl + "categories/sub/" + subcategory.id)
        .then((res) => {
          if (res.status === 200) {
            window.location.reload();
          } else {
  ...
  const handleUpdateSubcategory = (subcategory, newName) => {
    if (
      data?.find(
        (item) =>
          item.name === newName && item.category_id === subcategory.category_id
      )
    ) {
      return toast.warn("Trùng tên với tiểu danh mục đã tồn tại!");
    } else {
      axios
        .put(serverUrl + "categories/sub/" + subcategory.id, {
          name: newName,
        })
        .then((res) => {
          if (res.status === 200) {
  ...
  useEffect(() => {
    ...
    const apiUrl = categoryIdParam
      ? serverUrl + "categories/sub?category_id=" + categoryIdParam
      : serverUrl + "categories/sub";
