import { Box, Grid, IconButton, Rating, Stack, Typography } from '@mui/material'
import React from 'react'
import { formatVNDCurrency, getPriceAfterDiscount } from '../../utils/currencyUtils'
import { colors } from '../../services/const'

const ProductGridItem = ({ imageSrc, name, price, discount, rate, totalPurchase }) => {
  return (
    <Grid item xs={6} sm={4} md={3} lg={3} xl={2}>
        ...
        <Stack gap={{xs: '4px', md: '12px'}} sx={{ marginTop: '16px' }}>
            <Typography 
            title={name}
            sx={{ fontSize: {xs: '14px', md: '16px'}, fontWeight: 500, WebkitLineClamp: 2, textOverflow: 'ellipsis', overflow: 'hidden', textWrap: 'nowrap' }}
            >
                {name}
            </Typography>
            <Stack direction={'row'} gap={{xs: '4px', md: '12px'}} flexWrap={'wrap'}>
                <Typography sx={{ fontSize: {xs: '14px', md: '16px'}, fontWeight: 500 }}>
                    {formatVNDCurrency(getPriceAfterDiscount(price, discount))}
                </Typography>
                <Typography sx={{ fontSize: {xs: '14px', md: '16px'}, fontWeight: 400, textDecoration: 'line-through', opacity: 0.5 }}>
                    {formatVNDCurrency(price)}
                </Typography>
            </Stack>
            <Stack direction={'row'} alignItems={'end'} gap={'8px'}>
                <Rating name={`${name}-rating`} value={rate} precision={0.5} readOnly />
                <Typography 
                sx={{
                    fontSize: '14px',
                    fontWeight: 500
