...
const CategoryModal = ({ 
  ...
  useEffect(() => {
      if (type !== 'object' && type !== 'category' && type !== 'subcategory') {
        return console.error('Type of this CategoryModal must be in object, category or subcategory')
      }
  },[type])
  ...
  useEffect(() => {
    if (type === 'subcategory' && selectedObject) {
        ...
        axios.get(serverUrl + 'categories?object_id=' + selectedObject.id)
  ...
  return (
    ...
    >
        ...
        <Typography variant="h6" component="h2" sx={{ fontSize: {xs: '18px', md: '20px'} }}>
            {title}
        </Typography>
        {isEdit && (
            type === 'category' ? (
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} sx={{ mt: '16px' }} gap={'8px'}>
            <TextField id="new-name" label={inputLabel} variant="outlined" value={newName} onChange={(e) => setNewName(e.target.value)}/>
            <Button 
                variant='contained' 
                sx={{ flexGrow: 1 }} 
                disabled={disableBtn} 
...
const Selector = ({ disabled, label, setSelectedValue = (value) => {}, options = [] }) => {
